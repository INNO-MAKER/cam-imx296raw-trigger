###############################################################################################################
#Description: This codes was designed for help the user Automatic install the imx296 driver
...
#
#Author:      calvin , support@inno-maker.com 
#
#data:        2021.05.06
#
###############################################################################################################
...
echo "------INNO-MAKER cam-mipiimx296 driver install script  v1.0------"
...
END{
	print "count2 = "count2;
	if ( count <= 0 ) {
		print  "--- Add dtoverlay=vc_mipi_imx296 to /boot/config.txt";
		system("sudo sh -c \"sed -i.bak \"/^#dtoverlay=vc_mipi_imx296/d\" /boot/config.txt\" ");
		system("sudo sh -c \"echo \"dtoverlay=vc_mipi_imx296\" >> /boot/config.txt\" ");
	}
...
case $strVersion in
    *5.10.17*)
                version=Linux_5.10.17
        ;; 
	*5.4.51*)
                version=Linux_5.4.51
        ;;
...
case $strModel in
	...
	*Pi' '3*)
	...
	;;
	*Pi' '0*)
		echo "@ This driver do not support pi0, Please contact support@inno-maker.com"
	;;
	*)
...
echo "-----make setmode1----START:"
make setmode1
echo "-----make setmode1----END."
