...
aarch64 ``i2c_read`` / ``i2c_write`` / ``imx296_trigger`` C binaries and
bundles three high-level helpers customers actually use:

  * Chapter 4  - External trigger mode on the IMX296 sensor (slave 0x1a,
                 16-bit reg addr).  Single-command on/off; reproduces the
                 byte sequence of InnoMaker's standalone ``imx296_trigger``
                 binary, which the manual references but does not document.
...
Trigger (Pi5: bus 4 = CAM1, bus 6 = CAM0):
...
# -------------------------------------------------------------------- #
#  IMX296 external-trigger helpers
# -------------------------------------------------------------------- #
#  These six writes reproduce the byte sequence of InnoMaker's standalone
#  ``imx296_trigger`` ELF binary, obtained by disassembling that binary.
#  The manual references the tool (section 4.5) but does not document the
#  individual registers; semantic comments below are best-effort guesses.
#  Per the manual, run trigger commands while the camera stream is OFF.

# (reg, value) pairs.  Order matters - the standby toggle in the middle is
# what makes the new mode bits take effect.
_TRIGGER_ON_SEQ = [
    (0x300A, 0x01),   # register-hold start (atomic write begin)
    (0x3000, 0x01),   # standby on
    (0x3000, 0x00),   # standby off (commits sensor reset)
    (0x300B, 0x01),   # (enable bit, trigger-related)
    (0x30AE, 0x01),   # external trigger enable
    (0x300A, 0x00),   # register-hold end (atomic write end)
]
...
_TRIGGER_OFF_SEQ = [
    ...
    (0x3000, 0x01),
    (0x3000, 0x00),
    (0x300B, 0x00),   # disable bit
    (0x30AE, 0x00),   # external trigger disable
    (0x300A, 0x00),
]

# Registers worth reading back for `trigger show`.
_TRIGGER_REGS = (0x300A, 0x3000, 0x300B, 0x30AE)


def trigger_on(bus, addr=0x1A):
    """Put the IMX296 into external-trigger mode (XTR pin drives exposure)."""
    for reg, val in _TRIGGER_ON_SEQ:
        reg_write(bus, addr, reg, val, reg_bits=16)


def trigger_off(bus, addr=0x1A):
    """Take the IMX296 out of external-trigger mode (back to free-running)."""
    for reg, val in _TRIGGER_OFF_SEQ:
        reg_write(bus, addr, reg, val, reg_bits=16)


def trigger_show(bus, addr=0x1A):
    rows = []
    for r in _TRIGGER_REGS:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # --- trigger --------------------------------------------------------
    sp = sub.add_parser("trigger", help="IMX296 external-trigger helpers "
                                        "(replaces imx296_trigger binary)")
    ts = sp.add_subparsers(dest="trigger_cmd", required=True)

    ton = ts.add_parser("on", aliases=["enable"],
                        help="enter external-trigger mode")
    ton.add_argument("--bus",  type=_autoint, required=True,
                     help="i2c bus number (Pi5: 4 = CAM1, 6 = CAM0)")
    ton.add_argument("--addr", type=_autoint, default=IMX296_ADDR,
                     help=f"sensor slave addr (default 0x{IMX296_ADDR:02X})")

    toff = ts.add_parser("off", aliases=["disable"],
                         help="exit external-trigger mode (free-running)")
    toff.add_argument("--bus",  type=_autoint, required=True)
    toff.add_argument("--addr", type=_autoint, default=IMX296_ADDR)

    tsh = ts.add_parser("show", help="read back the 4 trigger-related registers")
    tsh.add_argument("--bus",  type=_autoint, required=True)
    tsh.add_argument("--addr", type=_autoint, default=IMX296_ADDR)
...
def _cmd_trigger_on(args) -> int:
    with I2CBus(args.bus) as bus:
        trigger_on(bus, addr=args.addr)
    print(f"trigger ON  : bus={args.bus}  addr=0x{_normalize_addr(args.addr):02X}")
    print("  Sensor will now wait for an external pulse on the XTR pin before each frame.")
    print("  ! Run this while the camera stream is OFF (no rpicam/libcamera/v4l2 running).")
    return 0


def _cmd_trigger_off(args) -> int:
    with I2CBus(args.bus) as bus:
        trigger_off(bus, addr=args.addr)
    print(f"trigger OFF : bus={args.bus}  addr=0x{_normalize_addr(args.addr):02X}")
    print("  Sensor is back to free-running mode.")
    return 0


def _cmd_trigger_show(args) -> int:
    with I2CBus(args.bus) as bus:
        rows = trigger_show(bus, addr=args.addr)
    print(f"trigger registers on bus={args.bus} addr=0x{_normalize_addr(args.addr):02X}:")
    hints = {
        0x300A: "  <- register-hold (should be 0x00 at rest)",
        0x3000: "  <- standby      (should be 0x00 at rest)",
        0x300B: "  <- 0x01 = trigger ON / 0x00 = OFF",
        0x30AE: "  <- 0x01 = trigger ON / 0x00 = OFF",
    }
    for reg, val in rows:
        print(f"  0x{reg:04X} = 0x{val:02X}{hints.get(reg, '')}")
    return 0
...
_DISPATCH = {
    ...
    ("trigger", "on"):             _cmd_trigger_on,
    ("trigger", "enable"):         _cmd_trigger_on,
    ("trigger", "off"):            _cmd_trigger_off,
    ("trigger", "disable"):        _cmd_trigger_off,
    ("trigger", "show"):           _cmd_trigger_show,
...
def main(argv=None) -> int:
    ...
    sub = (getattr(args, "trigger_cmd", None)
           or getattr(args, "strobe_cmd", None)
           or getattr(args, "eeprom_cmd", None))
